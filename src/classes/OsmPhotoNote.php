...
class OsmPhotoNote
{
    private const OSM_NOTES_API = 'https://api.openstreetmap.org/api/0.6/notes/';
    ...
    public function __construct($note_id)
    {
        ...
        $response = fetchUrl($note_fetch_url, getenv('OSM_API_USER'), getenv('OSM_API_PASS'));
        ...
        $search_regex = '~(?<!\S)' . preg_quote(trim(getenv('PHOTOS_SRV_URL'), '/')) . '/(\d+)\.[a-z]+(?!\S)~i';
